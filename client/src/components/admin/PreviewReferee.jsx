...
const PreviewReferee = ({ id }) => {
  const [inputValue, setInputValue] = useState("");
  const [toggle, setToggle] = useState(null);

  const [data, setData] = useState([]);
  const getData = async () => {
    const { data } = await axios.get(`${REFEREE_SERVER_URL}/view/${id}`);
    const { referee } = data;
    setData(referee);
    if (data.error) {
      alert(data.error);
  ...
  const handleDelete = async () => {
    const { data } = await axios.delete(`${REFEREE_SERVER_URL}/delete/${id}`);
    if (data.message) {
      swal({
        title: "Good job!",
        text: ` ${data.message} `,
                ...
                <span className="flex border border-gray-200">
                  <div className="flex items-center pl-4 border border-gray-200 rounded dark:border-gray-700">
                    <input
                      ...
                      value="female"
                      name="Sex"
                      defaultValue={data.Sex === "female" ? "female" : ""}
                  ...
                  <div className="flex items-center pl-4 border border-gray-200 rounded dark:border-gray-700">
                    <input
                      ...
                      value="male"
                      name="Sex"
                      defaultValue={data.Sex === "male" ? "male" : ""}
                      className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                    />
                    <label
                      htmlFor="Gender-1"
                  ...
                  <label className=" mx-3 text-end w-full py-4 ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                     Normal:  {data.Sex === "male" ? "male" : data.Sex}
                    </label>
